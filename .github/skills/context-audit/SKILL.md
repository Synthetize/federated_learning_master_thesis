...
name: context-audit
description: >
  Audit your Copilot CLI setup for token waste and context bloat. Use when
  the user says "audit my context", "check my settings", "why is Copilot so
  ...
  instruction files, skills, settings, and file permissions. Returns a
...
Read all Copilot instruction files:
- `.github/copilot-instructions.md`
- `.github/instructions/**/*.instructions.md`
- `AGENTS.md` (repo root and cwd)
- `$HOME/.copilot/copilot-instructions.md` (if accessible)

Count lines across these files. Then read every rule and test against five filters:

| Filter | Flag when... |
|--------|-------------|
| Default | Copilot already does this without being told ("write clean code", "handle errors") |
...
If total instruction-file lines > 200, check for progressive disclosure
...
with one-line pointers. Only recommend splitting when instruction files are
actually bloated -- lean universal instructions are fine as a single file.

### Skills

Scan `.github/skills/*/SKILL.md`. For each skill:
...
| Issue | Points |
|-------|--------|
| Copilot instruction files > 200 lines | -10 |
| Copilot instruction files > 500 lines | -20 |
...
- Show you cleaned-up Copilot instruction files with the flagged rules removed
...
Show diffs for instruction files and skills -- let the user confirm before
